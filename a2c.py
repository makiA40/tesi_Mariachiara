...
class MagazzinoEnv(gym.Env):
    ...
    def _calculate_distance(self, start: Tuple[int, int, int], end: Tuple[int, int, int]) -> float:
        ...
        return self.distanze[index_start, index_end] / 1000
...
def plot_individual_results(results, title, filename):
    metrics = ['ep_rew_mean', 'ep_len_mean', 'value_loss', 'policy_gradient_loss', 'vertical_distances', 'horizontal_distances']
    colors = ['b', 'g', 'r', 'c', 'm', 'y']
    titles = [
