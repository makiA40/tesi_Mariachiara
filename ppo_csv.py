...
class MagazzinoEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        self.logger.info(f"Vertical distance: {self.vertical_distance}, Horizontal distance: {self.horizontal_distance}")
    ...
    def export_distance_log(self, filename="distance_log_ppo.csv"):
        pd.DataFrame(self.distance_log).to_csv(filename, index=False)
...
def plot_individual_results(results, title, filename):
    ...
    colors = ['b', 'g', 'r', 'c', 'm', 'y']
    titles = [
        'PPO - Mean Reward per Episode', 'PPO - Mean Episode Length',
        'PPO - Value Loss', 'PPO - Policy Gradient Loss',
        'PPO - Vertical Distance per Episode', 'PPO - Horizontal Distance per Episode'
    ]
    ...
    for metric, color, sub_title, y_label in zip(metrics, colors, titles, y_labels):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"ppo_{filename.replace('.xlsx', '')}_{metric}.png")
        plt.show()
        plt.close()
...
def collect_results(env, model, total_timesteps):
    ...
    while timesteps < total_timesteps:
        ...
        print(
            f"Timesteps: {timesteps}, Vertical Distance: {env.total_vertical_distance}, Horizontal Distance: {env.total_horizontal_distance}")  # Debug line
    return results
...
plot_individual_results(trained_results, title="PPO Trained Model", filename="magazzino.xlsx")
...
# Esportazione del log delle distanze
env.export_distance_log("distance_log_ppo.csv")
